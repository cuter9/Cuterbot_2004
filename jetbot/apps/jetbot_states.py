...
class jetbot_states:
    ...
    def pwr_states(self, channel=BOARD):
        ...
        bus_voltage = self.pwr_sensor.getBusVoltage_V(channel)
        shunt_voltage = self.pwr_sensor.getShuntVoltage_mV(channel)
        # minus is to get the "sense" right.   - means the battery is charging, + that it is discharging
        current_mA = self.pwr_sensor.getCurrent_mA(channel)

        load_voltage = bus_voltage + (shunt_voltage / 1000)
        load_power = current_mA * load_voltage / 1000
        return {"in_volt": load_voltage,
                "in_current": current_mA,
